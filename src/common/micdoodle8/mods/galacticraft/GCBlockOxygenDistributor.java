...
public class GCBlockOxygenDistributor extends BlockContainer
{
    ...
    {
        par5EntityPlayer.openGui(Galacticraft.instance, GCConfigManager.idGuiAirDistributor, par1World, par2, par3, par4);
        return true;
    }
    	...
    	for (int j = -5; j < 6; j++)
		{
			for (int i = -5; i < 6; i++)
			{
				for (int k = -5; k < 6; k++)
				{
					...
					else if (world.getBlockId(x + i, y + j, z + k) == GCBlocks.unlitTorchLit.blockID)
					{
						int meta = world.getBlockMetadata(x + i, y + j, z + k);
						world.setBlockAndMetadataWithNotify(x + i, y + j, z + k, GCBlocks.unlitTorch.blockID, meta);
					}
				}
			}
		}
    ...
    public static void updateDistributorState(boolean activate, World par1World, int x, int y, int z)
    {
        ...
        if (activate)
        {
            par1World.setBlockWithNotify(x, y, z, GCBlocks.airDistributorActive.blockID);
        }
        else
        {
            par1World.setBlockWithNotify(x, y, z, GCBlocks.airDistributor.blockID);
            GCBlocks.airDistributor.removeAirBlocks(par1World, x, y, z);
        }
