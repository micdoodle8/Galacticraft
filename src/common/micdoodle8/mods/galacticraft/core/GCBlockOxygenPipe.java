...
public class GCBlockOxygenPipe extends BlockContainer
{
	private float oxygenPipeMin = 0.4F;
	private float oxygenPipeMax = 0.6F;
	
	public GCBlockOxygenPipe(int i, int j) 
	{
	...
	public void addCollidingBlockToList(World world, int i, int j, int k, AxisAlignedBB axisalignedbb, List arraylist, Entity par7Entity) 
	{
		...
		if (world.getBlockId(i - 1, j, k) == this.blockID || world.getBlockId(i - 1, j, k) == GCMarsBlocks.airDistributor.blockID || world.getBlockId(i - 1, j, k) == GCMarsBlocks.airDistributorActive.blockID)
		{
		...
		if (world.getBlockId(i, j - 1, k) == this.blockID || world.getBlockId(i, j - 1, k) == GCMarsBlocks.airDistributor.blockID || world.getBlockId(i, j - 1, k) == GCMarsBlocks.airDistributorActive.blockID)
		{
		...
		if (world.getBlockId(i, j, k - 1) == this.blockID || world.getBlockId(i, j, k - 1) == GCMarsBlocks.airDistributor.blockID || world.getBlockId(i, j, k - 1) == GCMarsBlocks.airDistributorActive.blockID)
		{
