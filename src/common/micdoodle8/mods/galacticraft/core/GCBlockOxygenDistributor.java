...
public class GCBlockOxygenDistributor extends BlockContainer
{
    ...
    public void removeAirBlocks(World world, int x, int y, int z)
    {
    	for (int j = -5; j < 6; j++)
		{
			for (int i = -5; i < 6; i++)
			{
				for (int k = -5; k < 6; k++)
				{
					if (world.getBlockId(x + i, y + j, z + k) == GCMarsBlocks.breatheableAir.blockID)
					{
					...
					else if (world.getBlockId(x + i, y + j, z + k) == GCMarsBlocks.unlitTorchLit.blockID)
					{
						int meta = world.getBlockMetadata(x + i, y + j, z + k);
						world.setBlockAndMetadataWithNotify(x + i, y + j, z + k, GCMarsBlocks.unlitTorch.blockID, meta);
					}
				}
			}
		}
    ...
    public static void updateDistributorState(boolean activate, World par1World, int x, int y, int z)
    {
        ...
        if (activate)
        {
            par1World.setBlockWithNotify(x, y, z, GCMarsBlocks.airDistributorActive.blockID);
        }
        else
        {
            par1World.setBlockWithNotify(x, y, z, GCMarsBlocks.airDistributor.blockID);
            GCMarsBlocks.airDistributor.removeAirBlocks(par1World, x, y, z);
        }
