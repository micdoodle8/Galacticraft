...
public class GCMarsPlayerBaseServer extends ServerPlayerBase
{
        ...
        for (int var9 = var3; var9 < var4; ++var9)
        {
            for (int var10 = var5; var10 < var6; ++var10)
            {
                for (int var11 = var7; var11 < var8; ++var11)
                {
                    ...
                    if (var12 != null && var12.blockID == GCMarsBlocks.breatheableAir.blockID)
                    {
    ...
    public void onUpdate()
	{
		if (player.dimension == GCMarsConfigManager.dimensionIDMars && player.inventory.getCurrentItem() != null && player.inventory.getCurrentItem().getItem().shiftedIndex == Block.torchWood.blockID)
        {
        	...
        	ItemStack stack = new ItemStack(GCMarsBlocks.unlitTorch, par1, 0);
        	
            player.inventory.mainInventory[player.inventory.currentItem] = stack;
        }
        else if (player.dimension != GCMarsConfigManager.dimensionIDMars && player.inventory.getCurrentItem() != null && player.inventory.getCurrentItem().getItem().shiftedIndex == GCMarsBlocks.unlitTorch.blockID)
        {
        	int par1 = player.inventory.getCurrentItem().stackSize;
...
//		FMLLog.info("" + Galacticraft.instance.tick % 10);
    ...
    public void sendAirRemainingPacket()
    {
        ...
        FMLCommonHandler.instance().getMinecraftServerInstance().getConfigurationManager().getPlayerForUsername(player.username).playerNetServerHandler.sendPacketToPlayer(GCMarsUtil.createPacket("Galacticraft", 0, toSend));
    }
