 ...
 *
 */
public class GCMarsComponentCreeperPitRoom extends GCStructureComponent
{
						...
						if (world.getBlockId(this.getBoundingBox().minX + x, this.getBoundingBox().minY + y, this.getBoundingBox().minZ + z) == GCMarsBlocks.creeperDungeonWall.blockID);
						{
							for (int i = -2; i < 2; i++)
							{
								for (int j = -2; j < 2; j++)
								{
									...
									{
										this.placeBlockAtCurrentPosition(world, GCMarsBlocks.creeperDungeonWall.blockID, 0, x + i, y, z + j, getBoundingBox());
									}
		                    		...
		                    		if (rand.nextInt(5) == 0 && world.getBlockId(this.getBoundingBox().minX + x + i, this.getBoundingBox().minY + y + 1, this.getBoundingBox().minZ + z + j) == 0 && world.getBlockId(this.getBoundingBox().minX + x + i, this.getBoundingBox().minY + y, this.getBoundingBox().minZ + z + j) == GCMarsBlocks.creeperDungeonWall.blockID)
		                    		{
										...
										this.placeBlockAtCurrentPosition(world, GCMarsBlocks.creeperEgg.blockID, 0, x + i, y + 1, z + j, getBoundingBox());
		                    		}
								}
							}
							...
							if (rand.nextInt(7) == 0)
							{
								...
								if (x > 0 && x < 7 && z > 0 && z < 7)
								{
									...
									{
										this.placeBlockAtCurrentPosition(world, GCMarsBlocks.creeperEgg.blockID, 0, x, y + 2, z, getBoundingBox());
									}
