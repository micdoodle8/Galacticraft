...
public class GCCoreGuiAirDistributor extends GuiContainer
{
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2)
    {
        ...
        status = "Oxygen Input: " + Math.round((this.distributor.getPower() * 600.0D) / this.distributor.MAX_OXYGEN * 1000.0D) / 10.0D + "%";
    ...
    {
    	if (this.distributor.getPower() > 1 && (this.distributor.wattsReceived > 0 || this.distributor.ic2Energy > 0))
    	{
    		return EnumColor.DARK_GREEN + "Active";
    	}

    	if (this.distributor.getPower() < 1)
    	{
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3)
	{
		...
		{
			final int scale = (int) ((double) this.distributor.getPower() / (double) 10.0D * 54.0D);
