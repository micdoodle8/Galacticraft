        ...
        if (this.player instanceof EntityPlayerMP)
        {
            for (int var12 = 0; var12 < this.player.worldObj.playerEntities.size(); ++var12)
            {
                ...
                if (var13.dimension == this.player.worldObj.provider.getDimension())
                {
                    ...
                    {
                        GalacticraftCore.packetPipeline.sendTo(new PacketSimple(EnumSimplePacket.C_SPAWN_SPARK_PARTICLES, FMLClientHandler.instance().getClient().theWorld.provider.getDimension(), new Object[] { this.pos }), var13);
                    }
                }
            }
        }
