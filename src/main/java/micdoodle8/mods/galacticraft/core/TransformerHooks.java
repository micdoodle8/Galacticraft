...
//        GL11.glTranslatef(0.0F, f1, 0.0F);
    ...
    public static void setCurrentBuffer(VertexBuffer buffer)
    {
    ...
    public static void addRainParticles(Random random, int rendererUpdateCount, float f)
    {
            ...
            if (canRain && blockpos1.getY() <= blockpos.getY() + i && blockpos1.getY() >= blockpos.getY() - i)
            {
                ...
                if (iblockstate.getMaterial() != Material.LAVA && iblockstate.getBlock() != Blocks.MAGMA)
                {
                    if (iblockstate.getMaterial() != Material.AIR)
                    {
                        ...
                        {
                            mc.effectRenderer.addEffect(((IWeatherProvider) world.provider).getParticle(mc.theWorld, x, y, z));
                        }
                        else
                        {
                            mc.theWorld.spawnParticle(EnumParticleTypes.WATER_DROP, x, y, z, 0.0D, 0.0D, 0.0D, new int[0]);
                        }
                    }
                }
                else
                {
                    mc.theWorld.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, blockpos1.getX() + xd, blockpos1.getY() + 0.1D - axisalignedbb.minY, blockpos1.getZ() + zd, 0.0D, 0.0D, 0.0D, new int[0]);
                }
            }
        }
        ...
        if (j > 0 && random.nextInt((world.provider instanceof IWeatherProvider) ? ((IWeatherProvider) world.provider).getSoundInterval(f) : 3) < rainSoundCounter++)
        {
            ...
            else
            {
                ...
                {
                    mc.theWorld.playSound(xx, yy, zz, SoundEvents.WEATHER_RAIN_ABOVE, SoundCategory.WEATHER, 0.1F, 0.5F, false);
                }
                else
                {
                    mc.theWorld.playSound(xx, yy, zz, SoundEvents.WEATHER_RAIN, SoundCategory.WEATHER, 0.2F, 1.0F, false);
                }
            }
        }
    }
