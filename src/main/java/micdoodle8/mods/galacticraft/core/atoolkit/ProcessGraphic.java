...
public class ProcessGraphic
{
	public static void go()
	{
		try
		{
			...
			if (toProc.exists() && toProc.isFile())
			{
				...
				BMPEncoder.write(processedImage (image), outFile);
			}
		}
		catch (IOException e)
		{
	...
	private static BufferedImage processedImage(BufferedImage image)
	{
		...
		for (int i = 0; i < height; i++)
		{
			...
			Image subimage = image.getSubimage(width-phi, i, phi*2, 1).getScaledInstance(width * 2, 1, BufferedImage.SCALE_SMOOTH);
