 ...
 * 
 */
public class SealerThreadManager
{
	...
	static
	{
		SealerThreadManager.vanillaPermeableBlocks.add(Blocks.sponge);
		...
		try
		{
			for (final String s : GCConfigManager.sealableIDs)
			{
				...
				if (SealerThreadManager.nonPermeableBlocks.containsKey(Integer.parseInt(split[0])))
				{
					final ArrayList<Integer> l = SealerThreadManager.nonPermeableBlocks.get(Integer.parseInt(split[0]));
					l.add(Integer.parseInt(split[1]));
					SealerThreadManager.nonPermeableBlocks.put(Integer.parseInt(split[0]), l);
				}
				else
				{
					final ArrayList<Integer> a = new ArrayList<Integer>();
					a.add(Integer.parseInt(split[1]));
					SealerThreadManager.nonPermeableBlocks.put(Integer.parseInt(split[0]), a);
				}
			}
		}
