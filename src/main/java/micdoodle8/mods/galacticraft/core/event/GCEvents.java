...
import micdoodle8.mods.galacticraft.api.event.oxygen.EventSuffocation;
...
public class GCEvents
{
	...
	public void onPlayerClicked(PlayerInteractEvent event)
	{
		...
		final Block blockBlocked = event.entityPlayer.worldObj.getBlock(event.x, event.y, event.z);
		...
		if (heldStack != null)
		{
			...
			if (event.entityPlayer.worldObj.provider instanceof IGalacticraftWorldProvider && heldStack.getItem() instanceof ItemFlintAndSteel)
			{
				...
				{
					if (blockBlocked != Blocks.tnt && !OxygenUtil.isAABBInBreathableAirBlock(event.entityLiving.worldObj, new Vector3(event.x, event.y, event.z), new Vector3(event.x + 1, event.y + 1, event.z + 1)))
					{
	...
	public void entityLivingEvent(LivingUpdateEvent event)
	{
		if (event.entityLiving.worldObj.provider instanceof IGalacticraftWorldProvider && !(event.entityLiving instanceof EntityPlayer) && !OxygenUtil.isAABBInBreathableAirBlock(event.entityLiving))
		{
			...
			{
				EventSuffocation suffocationEvent = new EventSuffocation.Pre(event.entityLiving);
				...
				event.entityLiving.attackEntityFrom(GCDamageSource.oxygenSuffocation, 1);

				EventSuffocation suffocationEventPost = new EventSuffocation.Post(event.entityLiving);
	...
	public ItemStack fillCustomBucket(World world, MovingObjectPosition pos)
	{
		...
		if ((blockAt == bcOil1 || blockAt == bcOil2 || blockAt == GCBlocks.crudeOilStill) && world.getBlockMetadata(pos.blockX, pos.blockY, pos.blockZ) == 0 && bcOilBucket != null)
		{
	...
	public void populate(PopulateChunkEvent.Post event)
	{
		...
		GCEvents.doPopulate(event.world, event.rand, worldX + event.rand.nextInt(16), worldZ + event.rand.nextInt(16));
	}
	...
	public void schematicFlipEvent(FlipPage event)
	{
		...
		{
		case 1:
			page = GCEvents.getNextSchematic(event.index);
			break;
		case -1:
			page = GCEvents.getLastSchematic(event.index);
			break;
		}
