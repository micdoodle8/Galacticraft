...
public class GCCoreGuiGalaxyMap extends GCCoreGuiStarBackground
{
	...
	protected void genAchievementBackground(int par1, int par2, float par3)
	{
		...
		if (this.selectedPlanet != null)
		{
			...
			final int width = this.fontRendererObj.getStringWidth(this.selectedPlanet.getMapObject().getSlotRenderer().getPlanetName());
			Gui.drawRect(this.width / 2 - width / 2 - 4, 4, this.width / 2 + width / 2 + 4, 15, textCol1);
			this.fontRendererObj.drawString(this.selectedPlanet.getMapObject().getSlotRenderer().getPlanetName(), this.width / 2 - width / 2, 6, textCol2, false);
				...
				if (planet != null)
				{
					for (final Map.Entry<IPlanet, ArrayList<IMoon>> e : GalacticraftCore.mapMoons.entrySet())
					{
						...
						for (IMoon moon : moonList)
						{
							...
							{
								this.fontRendererObj.drawString("Moon of " + planet.getMapObject().getSlotRenderer().getPlanetName(), this.width / 2 - width / 2 + width + 10, 6, textCol2, false);
							}
						}
					}
				}
	...
	private void drawInfoBox(int cx, int cy, IMapObject planet)
	{
		// for (final IMapPlanet planet : GalacticraftCore.mapPlanets)
		{
			...
			this.fontRendererObj.drawStringWithShadow(planet.getSlotRenderer().getPlanetName(), cx + MathHelper.floor_float(planet.getPlanetSize() * 15.0F) + 3, cy - 1, GCCoreUtil.convertTo32BitColor(255, 200, 200, 200));
			...
			for (int i = 0; i < WorldUtil.getPlayersOnPlanet(planet).size(); i++)
			{
				...
				this.fontRendererObj.drawStringWithShadow(String.valueOf(WorldUtil.getPlayersOnPlanet(planet).get(i)), cx + MathHelper.floor_float(planet.getPlanetSize() * 30.0F), cy + 1 + 10 * (i + 1), GCCoreUtil.convertTo32BitColor(255, 220, 220, 220));
