...
public class GuiRocketRefill extends GuiAdvancedContainer
{
    ...
    static
    {
        for (int i = 0; i < 4; i++)
        {
            GuiRocketRefill.rocketTextures[i] = new ResourceLocation(GalacticraftCore.ASSET_DOMAIN, "textures/gui/rocket_" + i * 18 + ".png");
        }
    }
    ...
    public void initGui()
    {
        ...
        this.infoRegions.add(new InfoRegion((this.width - this.xSize) / 2 + (((EntityTieredRocket) this.mc.thePlayer.ridingEntity).rocketType.getInventorySpace() == 2 ? 70 : 71), (this.height - this.ySize) / 2 + 6, 36, 40, oxygenDesc, this.width, this.height));
    }
    ...
    {
        this.mc.getTextureManager().bindTexture(GuiRocketRefill.rocketTextures[(this.rocketType.getInventorySpace() - 2) / 18]);
