...
public class GCCoreOverlayLander extends GCCoreOverlay
{
	...
	public static void renderLanderOverlay()
	{
		...
		if (GCCoreOverlayLander.minecraft.thePlayer.ridingEntity.motionY < -2.0)
		{
			...
			GCCoreOverlayLander.minecraft.fontRenderer.drawString(press1 + Keyboard.getKeyName(GCCoreKeyHandlerClient.spaceKey.getKeyCode()) + press2, width / 4 - GCCoreOverlayLander.minecraft.fontRenderer.getStringWidth(press1 + Keyboard.getKeyName(GCCoreKeyHandlerClient.spaceKey.getKeyCode()) + press2) / 2, height / 8, GCCoreUtil.convertTo32BitColor(alpha, alpha, alpha, alpha));
		}
		...
		{
			String string = StatCollector.translateToLocal("gui.lander.velocity") + ": " + Math.round(((EntityLander) GCCoreOverlayLander.minecraft.thePlayer.ridingEntity).motionY * 1000) / 100.0D + " " + StatCollector.translateToLocal("gui.lander.velocityu");
