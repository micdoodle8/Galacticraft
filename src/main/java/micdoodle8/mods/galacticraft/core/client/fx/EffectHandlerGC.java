...
public class EffectHandlerGC
{
	public static void spawnParticle(String particleID, Vector3 position, Vector3 motion, Vector3 color)
	{
		...
		if (mc != null && mc.renderViewEntity != null && mc.effectRenderer != null)
		{
			...
			{
				particle = new EntityFXRocketSmoke(mc.theWorld, position, motion, 1.0F, false);
			}
			else if (particleID.equals("whiteSmokeLaunched"))
			{
				particle = new EntityFXRocketSmoke(mc.theWorld, position, motion, 1.0F, true);
			}
			else if (particleID.equals("whiteSmokeLargeIdle"))
			{
				particle = new EntityFXRocketSmoke(mc.theWorld, position, motion, 2.5F, false);
			}
			else if (particleID.equals("whiteSmokeLargeLaunched"))
			{
				particle = new EntityFXRocketSmoke(mc.theWorld, position, motion, 2.5F, true);
			}
			else if (particleID.equals("launchFlameIdle"))
			{
				particle = new EntityFXRocketFlame(mc.theWorld, position, motion, 1F, false);
			}
			else if (particleID.equals("launchFlameLaunched"))
			{
				particle = new EntityFXRocketFlame(mc.theWorld, position, motion, 1F, true);
			}
			...
			if (dX * dX + dY * dY + dZ * dZ < viewDistance * viewDistance)
			{
				...
				{
					particle = new EntityFXOxygen(mc.theWorld, position, motion, color);
				}
			}
