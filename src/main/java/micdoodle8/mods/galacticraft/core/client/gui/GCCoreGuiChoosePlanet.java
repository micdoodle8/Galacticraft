...
public class GCCoreGuiChoosePlanet extends GuiScreen
{
	...
	protected void drawItemStackTooltip(HashMap<Integer, ToolTipEntry> itemList, int par2, int par3)
	{
		...
		if (!itemList.isEmpty())
		{
			...
			for (int j2 = 0; j2 < itemList.size(); ++j2)
			{
				...
				GCCoreGuiChoosePlanet.drawItems.renderItemAndEffectIntoGUI(this.fontRendererObj, this.mc.renderEngine, pair.stack, l, stringY - 4);
	...
	static GuiButton getSendButton(GCCoreGuiChoosePlanet par0GuiLanguage)
	{
		return par0GuiLanguage.sendButton;
	}

	static GuiButton getCreateSpaceStationButton(GCCoreGuiChoosePlanet par0GuiLanguage)
	{
