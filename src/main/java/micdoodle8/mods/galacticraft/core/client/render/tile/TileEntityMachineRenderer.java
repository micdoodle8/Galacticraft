    ...
    public void renderTileEntityAt(TileEntity tile, double x, double y, double z, float partialTickTime, int par9)
    {
        ...
        else if (tile.getBlockMetadata() >= BlockMachine2.OXYGEN_STORAGE_MODULE_METADATA && tile.getBlockMetadata() < BlockMachine2.OXYGEN_STORAGE_MODULE_METADATA + 4)
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                worldRenderer.addVertexWithUV(-0.188F, -0.25F + percent * 0.5F, -0.501F, 0.0F, percent);
                worldRenderer.addVertexWithUV(0.188F, -0.25F + percent * 0.5F, -0.501F, 1.0F, percent);
                worldRenderer.addVertexWithUV(0.188F, -0.25F, -0.501F, 1.0F, 0.0F);
                worldRenderer.addVertexWithUV(-0.188F, -0.25F, -0.501F, 0.0F, 0.0F);
                tessellator.draw();
