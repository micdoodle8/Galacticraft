...
public class BlockAirLockFrame extends BlockAdvancedTile
{
	...
	{
		if (par1IBlockAccess.getBlockMetadata(par2, par3, par4) >= BlockAirLockFrame.METADATA_AIR_LOCK_CONTROLLER)
		{
		...
		else
		{
			for (final ForgeDirection orientation : ForgeDirection.values())
			{
				if (orientation != ForgeDirection.UNKNOWN)
				{
					...
					if (blockVec.getBlock(par1IBlockAccess).equals(GCBlocks.airLockSeal))
					{
