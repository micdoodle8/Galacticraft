...
public class PacketSimple implements IPacket
{
	...
	public void handleClientSide(EntityPlayer player)
	{
		...
		case C_MOUNT_ROCKET:
			...
			player.addChatMessage(new ChatComponentText(Keyboard.getKeyName(KeyHandlerGC.openFuelGui.getKeyCode()) + "       - Inventory / Fuel"));
			break;
		...
		case C_UPDATE_GEAR_SLOT:
			...
			TickHandlerClient.playerItemData.add(gearData);
	...
	public void handleServerSide(EntityPlayer player)
	{
		...
		case S_OPEN_FUEL_GUI:
			if (player.ridingEntity instanceof EntityBuggy)
			{
				CoreUtil.openBuggyInv(playerBase, (EntityBuggy) player.ridingEntity, ((EntityBuggy) player.ridingEntity).getType());
			}
		...
		case S_RENAME_SPACE_STATION:
			final SpaceStationSaveData ssdata = SpaceStationSaveData.getStationData(playerBase.worldObj, (Integer) this.data.get(1), playerBase);
