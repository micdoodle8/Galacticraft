...
//    }
    ...
    public void onPlayerCloned(PlayerEvent.Clone event)
    {
        ...
        if (event.isWasDeath() && event.getOriginal() instanceof EntityPlayerMP)
        {
            ...
            if (event.getOriginal().worldObj.provider instanceof IGalacticraftWorldProvider)
            {
                ...
                if (timeA != null && (bb - timeA) < 1500)
                {
                    ...
                    event.getOriginal().addChatMessage(new TextComponentString(msg));
                }
...
//    }
        ...
        if (playerMP.ticksExisted % 20 == 0 && playerMP.worldObj.provider instanceof IGalacticraftWorldProvider)
        {
            if (((IGalacticraftWorldProvider) playerMP.worldObj.provider).shouldCorrodeArmor())
            {
                ...
                if (!valid)
                {
                    ...
                    {
                        if (armor != null && armor.getItem() instanceof ItemArmor && !(armor.getItem() instanceof IArmorCorrosionResistant))
                        {
    ...
    protected void sendPlanetList(EntityPlayerMP player, GCPlayerStats stats)
    {
        ...
        if (!temp.equals(stats.getSavedPlanetList()) || (player.ticksExisted % 100 == 0))
        {
            ...
            GalacticraftCore.packetPipeline.sendTo(new PacketSimple(EnumSimplePacket.C_UPDATE_DIMENSION_LIST, GCCoreUtil.getDimensionID(player.worldObj), new Object[] { PlayerUtil.getName(player), temp, canCreateStations }), player);
    ...
    public void onPlayerUpdate(EntityPlayerMP player)
    {
        ...
        if ((ConfigManagerCore.challengeSpawnHandling) && stats.getUnlockedSchematics().size() == 0)
        {
            ...
            ChunkPos pair = player.worldObj.getChunkFromChunkCoords(spawnPos.intX() >> 4, spawnPos.intZ() >> 4).getChunkCoordIntPair();
