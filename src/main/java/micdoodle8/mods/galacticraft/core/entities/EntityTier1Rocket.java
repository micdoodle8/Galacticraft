    ...
    protected void spawnParticles(boolean launched)
    {
        if (!this.isDead)
        {
            ...
            else if (this.ticksExisted % 2 == 0)
            {
                Object[] rider = new Object[] { riddenByEntity };
                y += 0.6D;
                GalacticraftCore.proxy.spawnParticle("launchFlameIdle", new Vector3(x2 + 0.4 - this.rand.nextDouble() / 10D, y, z2 + 0.4 - this.rand.nextDouble() / 10D), new Vector3(x1 + 0.7, y1 - 1D, z1 + 0.7), rider);
                GalacticraftCore.proxy.spawnParticle("launchFlameIdle", new Vector3(x2 - 0.4 + this.rand.nextDouble() / 10D, y, z2 + 0.4 - this.rand.nextDouble() / 10D), new Vector3(x1 - 0.7, y1 - 1D, z1 + 0.7), rider);
                GalacticraftCore.proxy.spawnParticle("launchFlameIdle", new Vector3(x2 - 0.4 + this.rand.nextDouble() / 10D, y, z2 - 0.4 + this.rand.nextDouble() / 10D), new Vector3(x1 - 0.7, y1 - 1D, z1 - 0.7), rider);
                GalacticraftCore.proxy.spawnParticle("launchFlameIdle", new Vector3(x2 + 0.4 - this.rand.nextDouble() / 10D, y, z2 - 0.4 + this.rand.nextDouble() / 10D), new Vector3(x1 + 0.7, y1 - 1D, z1 - 0.7), rider);
            }
