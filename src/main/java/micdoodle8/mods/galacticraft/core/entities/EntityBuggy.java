...
public class EntityBuggy extends Entity implements IInventory, IPacketReceiver, IDockable, IControllableEntity
{
	...
	public boolean interactFirst(EntityPlayer var1)
	{
		if (this.worldObj.isRemote)
		{
			if (this.riddenByEntity == null)
			{
				var1.addChatMessage(new ChatComponentText(Keyboard.getKeyName(KeyHandlerGC.leftKey.getKeyCode()) + " / " + Keyboard.getKeyName(KeyHandlerGC.rightKey.getKeyCode()) + "  - " + StatCollector.translateToLocal("gui.buggy.turn.name")));
				var1.addChatMessage(new ChatComponentText(Keyboard.getKeyName(KeyHandlerGC.accelerateKey.getKeyCode()) + "       - " + StatCollector.translateToLocal("gui.buggy.accel.name")));
				var1.addChatMessage(new ChatComponentText(Keyboard.getKeyName(KeyHandlerGC.decelerateKey.getKeyCode()) + "       - " + StatCollector.translateToLocal("gui.buggy.decel.name")));
				var1.addChatMessage(new ChatComponentText(Keyboard.getKeyName(KeyHandlerGC.openFuelGui.getKeyCode()) + "       - " + StatCollector.translateToLocal("gui.buggy.inv.name")));
			}
