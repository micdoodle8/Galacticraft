...
public class WorldUtil
{
	...
	public static double getGravityForEntity(Entity eLiving)
	{
		if (eLiving.worldObj.provider instanceof IGalacticraftWorldProvider)
		{
			...
			{
				if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT && eLiving instanceof GCEntityClientPlayerMP)
				{
					return ((GCEntityClientPlayerMP) eLiving).touchedGround ? 0.08D - customProvider.getGravity() : 0.08D;
				}
				else if (eLiving instanceof GCEntityPlayerMP)
				{
					return ((GCEntityPlayerMP) eLiving).isTouchedGround() ? 0.08D - customProvider.getGravity() : 0.08D;
				}
				else
				{
					return 0.08D;
	...
	public static SpaceStationSaveData bindSpaceStationToNewDimension(World world, GCEntityPlayerMP player)
	{
	...
	private static Entity teleportEntity(World worldNew, Entity entity, int dimID, ITeleportType type, boolean transferInv, EntityAutoRocket ridingRocket)
	{
		...
		if (entity instanceof GCEntityPlayerMP)
		{
			...
			if (dimChange)
			{
				...
				// Packet9Respawn(player.dimension, (byte)
				// player.worldObj.difficultySetting,
				// worldNew.getWorldInfo().getTerrainType(),
				// worldNew.getHeight(),
				...
				if (worldNew.provider instanceof WorldProviderSpaceStation)
				{
					...
					SpaceStationSaveData.getStationData(worldNew, dimID, player).writeToNBT(var2);
					...
					// player.playerNetServerHandler.sendPacketToPlayer(GCCorePacketSpaceStationData.buildSpaceStationDataPacket(worldNew,
					// worldNew.provider.dimensionId, player));
				}
		...
		if (entity instanceof GCEntityPlayerMP && dimChange)
		{
			...
			// player.experienceLevel)); TODO Find out if this is still needed
		}
	...
	public static boolean canBlockPass(World world, Block block, int metadata, VecDirPair pair)
	{
		...
		if (SealerThreadManager.vanillaPermeableBlocks.contains(block))
		{
			return true;
		}
		...
		if (!block.isOpaqueCube())
		{
			...
			if (SealerThreadManager.nonPermeableBlocks.containsKey(block) && SealerThreadManager.nonPermeableBlocks.get(block).contains(metadata))
			{
				return false;
			}
	...
	{
		return WorldUtil.canBlockPass(world, pair.getPosition().getBlock(world), pair.getPosition().getBlockMetadata(world), pair);
	}
	...
	public static TileEntity[] getAdjacentOxygenConnections(TileEntity tile)
	{
		...
		for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS)
		{
			...
			// {
			// if (tileEntity instanceof ITubeConnection && (!(tileEntity
			// instanceof IGasTransmitter) ||
			// TransmissionType.checkTransmissionType(tileEntity,
			// TransmissionType.GAS, tileEntity)))
			// {
	...
	public static TileEntity[] getAdjacentPowerConnections(TileEntity tile)
	{
		...
		for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS)
		{
			...
			// tileEntity instanceof IEnergyTile)
			// {
			// if (tileEntity instanceof IEnergyAcceptor)
			// {
			// if (((IEnergyAcceptor) tileEntity).acceptsEnergyFrom(tile,
			...
			// {
			// if (((IEnergyEmitter) tileEntity).emitsEnergyTo(tileEntity,
			// direction.getOpposite()))
			// {
			// adjacentConnections[direction.ordinal()] = tileEntity;
			// continue;
			...
			// adjacentConnections[direction.ordinal()] = tileEntity;
			// }
			// else if (NetworkConfigHandler.isBuildcraftLoaded() && tileEntity
			// instanceof IPowerReceptor)
			// {
			// if (((IPowerReceptor)
			// tileEntity).getPowerReceiver(direction.getOpposite()) != null)
			// {
			// adjacentConnections[direction.ordinal()] = tileEntity;
			// }
			// }
			// else if (NetworkConfigHandler.isThermalExpansionLoaded() &&
			// tileEntity instanceof IEnergyHandler)
			// {
			// if (((IEnergyHandler)
			// tileEntity).canInterface(direction.getOpposite()))
			// {
			// adjacentConnections[direction.ordinal()] = tileEntity;
			// }
			// } TODO Re-implement when APIs are ready
