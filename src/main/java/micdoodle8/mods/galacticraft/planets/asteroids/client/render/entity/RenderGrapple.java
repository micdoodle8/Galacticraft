...
public class RenderGrapple extends Render
{
    public void doRender(EntityGrapple grapple, double x, double y, double z, float par8, float partialTicks)
    {
        ...
        {
            double d3 = shootingEntity.prevPosX + (shootingEntity.posX - shootingEntity.prevPosX) * (double)partialTicks + vec3.xCoord;
            double d4 = shootingEntity.prevPosY + (shootingEntity.posY - shootingEntity.prevPosY) * (double)partialTicks + vec3.yCoord;
            double d5 = shootingEntity.prevPosZ + (shootingEntity.posZ - shootingEntity.prevPosZ) * (double)partialTicks + vec3.zCoord;
            ...
            double d14 = grapple.prevPosX + (grapple.posX - grapple.prevPosX) * (double)partialTicks;
            double d8 = grapple.prevPosY + (grapple.posY - grapple.prevPosY) * (double)partialTicks + 0.25D;
            double d10 = grapple.prevPosZ + (grapple.posZ - grapple.prevPosZ) * (double)partialTicks;
            ...
            for (int i = 0; i <= b2; ++i)
            {
                ...
                tessellator.addVertex(x + d11 * (double)f12, y + d12 * (double)(f12 * f12 + f12) * 0.5D + 0.15D, z + d13 * (double)f12);
            }
