...
public class EntityTier3Rocket extends EntityTieredRocket
{
    ...
    public void onTeleport(EntityPlayerMP player)
    {
        ...
        if (playerBase != null)
        {
            ...
            stats.setRocketItemstack(new ItemStack(AsteroidsItems.tier3Rocket, 1, this.rocketType.getIndex()));
...
//            stats.setRocketItem(AsteroidsItems.tier3Rocket);
    ...
    private void makeFlame(double x2, double y2, double z2, Vector3 motionVec, boolean getLaunched, Object[] rider)
    {
        ...
        GalacticraftCore.proxy.spawnParticle("launchFlameIdle", new Vector3(x2 + 0.4 - this.rand.nextDouble() / 10D, y2, z2 + 0.4 - this.rand.nextDouble() / 10D), new Vector3(x1 + 0.5D, y1 - 0.3D, z1 + 0.5D), rider);
        GalacticraftCore.proxy.spawnParticle("launchFlameIdle", new Vector3(x2 - 0.4 + this.rand.nextDouble() / 10D, y2, z2 + 0.4 - this.rand.nextDouble() / 10D), new Vector3(x1 - 0.5D, y1 - 0.3D, z1 + 0.5D), rider);
        GalacticraftCore.proxy.spawnParticle("launchFlameIdle", new Vector3(x2 - 0.4 + this.rand.nextDouble() / 10D, y2, z2 - 0.4 + this.rand.nextDouble() / 10D), new Vector3(x1 - 0.5D, y1 - 0.3D, z1 - 0.5D), rider);
        GalacticraftCore.proxy.spawnParticle("launchFlameIdle", new Vector3(x2 + 0.4 - this.rand.nextDouble() / 10D, y2, z2 - 0.4 + this.rand.nextDouble() / 10D), new Vector3(x1 + 0.5D, y1 - 0.3D, z1 - 0.5D), rider);
