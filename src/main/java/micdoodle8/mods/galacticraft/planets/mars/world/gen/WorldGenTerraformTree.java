...
public class WorldGenTerraformTree extends Feature
{
                    ...
                    for (l = 0; l < i; ++l)
                    {
                        ...
                        if (block2.isAir(state2, worldIn, upN) || block2.isLeaves(state2, worldIn, upN) || block2.getMaterial(state2) == Material.VINE)
                        {
                            ...
                            if (this.vinesGrow && l > 0)
                            {
                                ...
                                {
                                    this.func_181647_a(worldIn, position.add(-1, l, 0), VineBlock.EAST);
                                }

                                if (rand.nextInt(3) > 0 && worldIn.isAirBlock(position.add(1, l, 0)))
                                {
                                    this.func_181647_a(worldIn, position.add(1, l, 0), VineBlock.WEST);
                                }

                                if (rand.nextInt(3) > 0 && worldIn.isAirBlock(position.add(0, l, -1)))
                                {
                                    this.func_181647_a(worldIn, position.add(0, l, -1), VineBlock.SOUTH);
                                }

                                if (rand.nextInt(3) > 0 && worldIn.isAirBlock(position.add(0, l, 1)))
                                {
                                    this.func_181647_a(worldIn, position.add(0, l, 1), VineBlock.NORTH);
                                }
                            }
                        }
                    }
