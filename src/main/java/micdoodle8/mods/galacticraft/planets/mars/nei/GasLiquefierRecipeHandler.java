...
public class GasLiquefierRecipeHandler extends TemplateRecipeHandler
{
	...
	public void drawBackground(int i)
	{
		...
		else if (progress < 104)
		{
			...
			if (this.fillAtmos) yoffset = 13 + level / 3;
			GuiDraw.changeTexture(GasLiquefierRecipeHandler.liquefierGasesTexture);
			GuiDraw.drawTexturedModalRect(2, 62 - yoffset, 1 + this.inputGas * 17, 26 - yoffset, 16, yoffset);
	...
	public ArrayList<PositionedStack> getIngredientStacks(int recipe)
	{
		...
		if (inputItem == AsteroidsItems.methaneCanister) this.inputGas = 0;
		else if (inputItem == AsteroidsItems.canisterLOX) this.inputGas = 1;
		else if (inputItem == AsteroidsItems.atmosphericValve) this.fillAtmos = true;
	...
	public PositionedStack getResultStack(int recipe)
	{
		...
		if (outputItem == GCItems.fuelCanister) this.outputGas = 0;
		else if (outputItem == AsteroidsItems.canisterLOX) this.outputGas = 3;
		else this.outputGas = 4;
