 ...
 * 
 */
public class TileEntityLaunchController extends TileEntityElectricBlock implements IChunkLoader, IElectrical, IInventory, ISidedInventory, IPacketReceiver, ILandingPadAttachable
{
	...
	{
		if (!this.worldObj.isRemote && ConfigManagerPlanets.launchControllerChunkLoad)
		{
			if (ticket == null)
			{
				return;
			...
			for (int x = -2; x <= 2; x++)
			{
				for (int z = -2; z <= 2; z++)
				{
					Block block = this.worldObj.getBlock(this.xCoord + x, this.yCoord, this.zCoord + z);
