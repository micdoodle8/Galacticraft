...
public class TileEntitySolarArrayController extends TileBaseUniversalElectricalSource implements IDisableableMachine, IInventoryDefaults, ISidedInventory, IConnector
{
                ...
                if (this.worldObj.isDaytime() && (this.worldObj.provider instanceof IGalacticraftWorldProvider || !this.worldObj.isRaining() && !this.worldObj.isThundering()))
                {
                    for (ITransmitter transmitter : solarArray)
                    {
                        ...
                        if (Math.abs(diff.getX()) <= 16 && diff.getY() == 0 && Math.abs(diff.getZ()) <= 16)
                        {
                            ...
                            if (this.worldObj.canBlockSeeSky(tile.getPos()))
                            {
                                boolean valid = true;

                                for (int y = this.getPos().getY() + 1; y < 256; y++)
                                {
                                    Block block = this.worldObj.getBlockState(this.getPos().add(0, y, 0)).getBlock();
    ...
    public float getSolarBoost()
    {
        ...
        if (this.worldObj.provider instanceof WorldProviderVenus)
        {
            if (this.pos.getY() > 90)
            {
