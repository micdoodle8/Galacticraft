		...
		if (par4 >= 1 && par4 + l + 1 <= 256)
		{
				...
				for (int l1 = par3 - b0; l1 <= par3 + b0 && flag; ++l1)
				{
					for (j1 = par5 - b0; j1 <= par5 + b0 && flag; ++j1)
					{
						if (i1 >= 0 && i1 < 256)
						{
							...
							if (!isAir && block != Blocks.grass && block != Blocks.water && block != Blocks.flowing_water && block != GCMarsBlocks.marsBlock && k2 != 5)
							{
								flag = false;
							}
						}
			...
			else
			{
				...
				if (par4 < 256 - l - 1)
				{
					...
					for (j1 = par4 - b0 + l; j1 <= par4 + l; ++j1)
					{
						...
						for (j2 = par3 - i2; j2 <= par3 + i2; ++j2)
						{
							...
							for (int l2 = par5 - i2; l2 <= par5 + i2; ++l2)
							{
								...
								{
									Block j3 = par1World.getBlock(j2, j1, l2);

									if (j3 == null || j3.canBeReplacedByLeaves(par1World, j2, j1, l2))
									{
					...
					for (j1 = 0; j1 < l; ++j1)
					{
						block = par1World.getBlock(par3, par4 + j1, par5);
					...
					if (this.vinesGrow)
					{
						for (j1 = par4 - 3 + l; j1 <= par4 + l; ++j1)
						{
							...
							for (j2 = par3 - i2; j2 <= par3 + i2; ++j2)
							{
								...
								{
									block = par1World.getBlock(j2, j1, k2);
