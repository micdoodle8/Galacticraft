 ...
 * 
 */
public class GCMarsRoomSpawner extends DungeonRoom
{
	int sizeX;
	int sizeY;
	int sizeZ;
	...
	public GCMarsRoomSpawner(MapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
	...
	protected void handleTileEntities(Random rand)
	{
		for (final ChunkCoordinates spawnerCoords : this.spawners)
		{
			if (this.worldObj.getBlock(spawnerCoords.posX, spawnerCoords.posY, spawnerCoords.posZ) == Blocks.mob_spawner)
			{
				...
				if (spawner != null)
				{
					spawner.func_145881_a().setEntityName(GCMarsRoomSpawner.getMob(rand));
				}
			}
		}
	}
