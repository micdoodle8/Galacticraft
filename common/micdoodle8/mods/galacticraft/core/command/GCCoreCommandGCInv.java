...
public class GCCoreCommandGCInv extends CommandBase
{
            ...
            try
            {
                ...
                if (thePlayer != null && !thePlayer.isDead && thePlayer.worldObj!=null)
                {
                	...
                	{	
                		...
                		savedata.put(astring[1].toLowerCase(),saveinv);
                	...
                	{	
                		ItemStack[] saveinv = savedata.get(astring[1].toLowerCase());
                ...
                {
                	//Special rule for 'restore' command if player not found - look to see if the player is offline (i.e. had a saved inventory already)
                	if (astring[0].equalsIgnoreCase("restore"))
                	{	
                		ItemStack[] saveinv = savedata.get(astring[1].toLowerCase());
                		if (saveinv!=null)
                		{
                			...
                			dontload.remove(astring[1].toLowerCase());  //Now it can autoload on next player logon
                			return;
                		}
                	}
    ...
    public static void doLoad(GCCorePlayerMP thePlayer)
    {
    	...
    	if (!dontload.contains(theName))  //This is a simple flag: if the playername is in dontload then no restore command has yet been run.
    		  //Dontload resets to nothing on server restart so that all will auto-restore on a server restart.
    	{	
    ...
    {
    	savefile.writeToNBT(new NBTTagCompound());
    	savefile.markDirty();
    }
    ...
    private static void initialise()
    {
    	...
    	savefile = (GCInvSaveData) world0.loadItemData(GCInvSaveData.class, "GCInv_savefile");
    	if (savefile==null)
    	{	
    		savefile = new GCInvSaveData("GCInv_savefile");
    		world0.setItemData("GCInv_savefile", savefile);
    	}
     }
