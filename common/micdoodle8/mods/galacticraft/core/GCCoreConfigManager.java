...
public class GCCoreConfigManager
{
	...
	public static void setDefaultValues(File file)
	{
		...
		try
		{
			...
			GCCoreConfigManager.disableSpaceStationCreation = GCCoreConfigManager.configuration.get(Configuration.CATEGORY_GENERAL, "Disable Space Station creation", false, "If set to true on a server, players will be completely unable to create space stations.").getBoolean(false);
			...
			GCCoreConfigManager.enableSealerMultithreading = GCCoreConfigManager.configuration.get(Configuration.CATEGORY_GENERAL, "Enable Sealer Multithreading", false, "(Experimental) If this is enabled, Oxygen Sealers seal checks will run in a separate thread - faster but there may be some artifacts.").getBoolean(false);
			GCCoreConfigManager.enableSealerEdgeChecks = GCCoreConfigManager.configuration.get(Configuration.CATEGORY_GENERAL, "Enable Sealed edge checks", true, "If this is enabled, areas sealed by Oxygen Sealers will run a seal check when the player breaks or places a block (or on block updates).  This should be enabled for a 100% accurate sealed status is accurate, but can be disabled on servers for performance reasons.").getBoolean(true);
