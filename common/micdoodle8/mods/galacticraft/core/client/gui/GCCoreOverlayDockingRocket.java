...
public class GCCoreOverlayDockingRocket extends GCCoreOverlay
{
    ...
    public static void renderDockingOverlay()
    {
        ...
        if (minecraft.thePlayer.ridingEntity instanceof EntityAutoRocket)
        {
            EntityAutoRocket rocket = (EntityAutoRocket) minecraft.thePlayer.ridingEntity;
            ...
            if (rocket.landing && rocket.targetVec != null)
            {
                ...
                if (dMY > 25)
                    GCCoreOverlayDockingRocket.minecraft.fontRenderer.drawString(warning, width / 2 - minecraft.fontRenderer.getStringWidth(warning) / 2, height / 3 - 50, green);
                
                if (dMY < -25)
                    GCCoreOverlayDockingRocket.minecraft.fontRenderer.drawString(warning2, width / 2 - minecraft.fontRenderer.getStringWidth(warning2) / 2, height / 3 - 35, red);

                GCCoreOverlayDockingRocket.minecraft.fontRenderer.drawString(targetMotionYStr, width - minecraft.fontRenderer.getStringWidth(targetMotionYStr) - 50, height / 3 + 50, grey);
                GCCoreOverlayDockingRocket.minecraft.fontRenderer.drawString(currentMotionYStr, width - minecraft.fontRenderer.getStringWidth(currentMotionYStr) - 50, height / 3 + 35, grey);
                ...
                GCCoreOverlayDockingRocket.minecraft.fontRenderer.drawString("Y: " + dYStr, 50, height / 3 + 45, (Math.abs(dY) > 50 || Math.abs(dY) < 1.9 ? grey : (screenTicks / 10 % 2 == 0 ? red : grey)));
